...
function setStatus(text, kind='') {
  const el = $('status'); el.textContent = text; el.className = `status ${kind}`;
}

function renderHistory() {
  const el = $('history');
  if (!state.history.length) { el.textContent = 'No generated songs yet.'; return; }
  el.innerHTML = state.history.slice(0, 20).map((x) => `
    <div class="history-item">
      <div><strong>${escapeHtml(x.title)}</strong><small>${escapeHtml(x.language)} · ${escapeHtml(x.genre)} · ${escapeHtml(x.voice)}</small></div>
      <button data-url="${escapeAttr(x.url)}">Play</button>
    </div>`).join('');
  el.querySelectorAll('button').forEach(b => b.onclick = () => { $('player').src = b.dataset.url; $('result').classList.remove('hidden'); $('player').play().catch(()=>{}); });
}
...
async function checkHealth() {
  try {
    ...
    $('modeBadge').textContent = j.demo_mode ? 'DEMO MODE' : 'AI ENGINE READY';
    $('modeBadge').classList.toggle('live', !j.demo_mode);
  ...
  state.timer = setInterval(async () => {
    try {
      ...
      if (j.status === 'completed') {
        ...
        $('player').src = url; $('download').href = url; $('result').classList.remove('hidden');
        $('meta').textContent = j.demo ? 'Demo audio. Connect ACE-Step for real AI music.' : 'Your AI-generated creation is ready.';
...
$('generate').onclick = async () => {
  ...
  if (!prompt && !lyrics) { setStatus('Please enter a song idea or lyrics.', 'error'); return; }
  $('generate').disabled = true; setStatus('Starting music generation…');
  try {
    const payload = { prompt, lyrics, language: $('language').value, genre: $('genre').value, mood: $('mood').value, voice: $('voice').value, duration: Number($('duration').value), format: 'mp3' };
    ...
    if (j.status === 'completed') { await poll(j.task_id); } else { await poll(j.task_id); }
